...
public class TabelaConfeccao extends JInternalFrame{
	...
	private static TabelaConfeccao tabelaConfeccao;
	
	public static  TabelaConfeccao getInstancia() {
		
		if(Objects.isNull(tabelaConfeccao)) {
			tabelaConfeccao = new TabelaConfeccao();
		}
		
		return tabelaConfeccao;
	}
	

	private TabelaConfeccao() {
		setTitle("Tabela Confeccao");
		//setIconImage(Toolkit.getDefaultToolkit().getImage(TabelaConfeccao.class.getResource("/imagens/list.png")));
	...
	private void CreatEvents() {
		...
		btnRelatorio.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ConfeccaoReports ConfeccaoReports = new ConfeccaoReports(totalRegistros);
				ConfeccaoReports.setLocationRelativeTo(null);
				ConfeccaoReports.setVisible(ProcessamentoDados.VERDADEIRO);
			}
		});
	...
	private void incluirNovoRegistro() {
		ConfeccaoFrame ConfeccaoFrame = new ConfeccaoFrame(new JFrame(), true, ProcessamentoDados.INCLUIR, tabelaConfeccaos, tabelaConfeccaoModel, 0);
	...
	protected void alterarRegistro() {
		...
		if(tabelaConfeccaos.getSelectedRow() != -1 && tabelaConfeccaos.getSelectedRow() < tabelaConfeccaoModel.getRowCount()) {
			linha = tabelaConfeccaos.getSelectedRow();
			ConfeccaoFrame ConfeccaoFrame = new ConfeccaoFrame(new JFrame(), true, ProcessamentoDados.ALTERAR, tabelaConfeccaos, tabelaConfeccaoModel, linha);
			ConfeccaoFrame.setLocationRelativeTo(null);
			ConfeccaoFrame.setVisible(true);
		}
		else{
	...
	protected void deletarRegistro() {
		...
		if(tabelaConfeccaos.getSelectedRow() != -1 && tabelaConfeccaos.getSelectedRow() < tabelaConfeccaoModel.getRowCount()) {
			linha = tabelaConfeccaos.getSelectedRow();
			ConfeccaoFrame ConfeccaoFrame = new ConfeccaoFrame(new JFrame(), true, ProcessamentoDados.EXCLUIR, tabelaConfeccaos, tabelaConfeccaoModel, linha);
			ConfeccaoFrame.setLocationRelativeTo(null);
			ConfeccaoFrame.setVisible(true);
		}
		else{
	...
	private void consultarRegistros() {
		...
		if(tabelaConfeccaos.getSelectedRow() != -1 && tabelaConfeccaos.getSelectedRow() < tabelaConfeccaoModel.getRowCount()) {
			linha = tabelaConfeccaos.getSelectedRow();
			ConfeccaoFrame ConfeccaoFrame = new ConfeccaoFrame(new JFrame(), true, ProcessamentoDados.CONSULTAR, tabelaConfeccaos, tabelaConfeccaoModel, linha);
			ConfeccaoFrame.setLocationRelativeTo(null);
			ConfeccaoFrame.setVisible(true);
		}
		else{
	...
	protected void filtraNomeConfeccao() {
		List<Confeccao> listaConfeccao = new ArrayList<Confeccao>();
		
		listaConfeccao = ConfeccaoService.mostrarConfeccao(textFieldBuscarNome.getText());
		...
		tabelaConfeccaos.setRowSorter(sortTabelaConfeccao);
    }
	...
	private void iniciarPaginacao() {
		...
		totalRegistros = buscarTotalRegistrosConfeccao();
		    ...
		    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		        ...
		        if ("Terminado".equals(status)) {
		            ...
		            cor.setForeground(ProcessamentoDados.VERMELHO);
				        ...
				        cor.setForeground(ProcessamentoDados.AMARELO);
		        		}
		        		else cor.setForeground(Color.black);
		        	}
		        }
		        return cor;
		    }
		...
		listaConfeccao = carregarListaConfeccao(paginaAtual, registroPorPagina);
		...
		tabelaConfeccaos.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		...
		tabelaConfeccaos.setAutoCreateRowSorter(true);
		tabelaConfeccaos.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
		tabelaConfeccaoModel.fireTableDataChanged();
		
		tabelaConfeccaos.getColumnModel().getColumn(CODIGO).setPreferredWidth(5);
		tabelaConfeccaos.getColumnModel().getColumn(ESTADO).setPreferredWidth(20);
		tabelaConfeccaos.getColumnModel().getColumn(INICIO).setPreferredWidth(20);
		tabelaConfeccaos.getColumnModel().getColumn(FIM).setPreferredWidth(20);
		
		
		tabelaConfeccaos.getColumnModel().getColumn(CODIGO).setCellRenderer(ProcessamentoDados.alinharColunaCentro());
		tabelaConfeccaos.getColumnModel().getColumn(INICIO).setCellRenderer(ProcessamentoDados.alinharColunaCentro());
		tabelaConfeccaos.getColumnModel().getColumn(FIM).setCellRenderer(ProcessamentoDados.alinharColunaCentro());
		tabelaConfeccaos.getColumnModel().getColumn(ESTADO).setCellRenderer(renderer);
		//scrollPane.setViewportView(tabelaConfeccaos);	
	}

	private List<Confeccao> carregarListaConfeccao(Integer paginaAtual2, Integer registroPorPagina2) {
		
		ConfeccaoService ConfeccaoService = new ConfeccaoService();
		return ConfeccaoService.carregarListaConfeccao(((paginaAtual-1)*registroPorPagina), registroPorPagina);
	}
	...
	private Integer buscarTotalRegistrosConfeccao() {
		return ConfeccaoService.countTotalRegistroConfeccaos();
	}
	...
	private void initComponents() {
		...
		tabelaConfeccaos = new JTable();
		scrollPane_1.setViewportView(tabelaConfeccaos);
		...
		btnPrimeiro.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/go-first.png")));
		...
		btnAnterior.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/go-previous.png")));
		...
		btnProximo.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/go-next.png")));
		...
		btnUltima.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/go-last.png")));
		...
		btnInclusao.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/book_add.png")));
		...
		btnAlteração.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/book_edit.png")));
		...
		btnExclusao.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/book_delete.png")));
		...
		btnConsultar.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/book_open.png")));
		...
		btnFechar.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/sair.png")));
		...
		btnRelatorio.setIcon(new ImageIcon(TabelaConfeccao.class.getResource("/imagens/pdf.png")));
